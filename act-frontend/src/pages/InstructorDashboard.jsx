          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
               <Link
                ...
                >
                  ...
                  <span className="font-semibold text-foreground">Create Course</span>
                  <span className="text-xs text-muted-foreground mt-1">Upload video and lessons</span>
               </Link>
               <Link
                ...
                >
                  ...
                  <span className="font-semibold text-foreground">Create Quiz</span>
                  <span className="text-xs text-muted-foreground mt-1">Set up new assessment</span>
               </Link>
            </div>

            <div className="rounded-2xl bg-card border border-border p-4">
              <div className="flex items-center justify-between mb-3">
                 <h2 className="font-semibold text-foreground">My Uploaded Courses</h2>
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-muted-foreground flex flex-wrap items-center gap-2 mt-0.5">
                           <span className="px-1.5 py-0.5 rounded bg-secondary text-secondary-foreground">{c.level || c.difficulty || 'Beginner'}</span>
                           <span>•</span>
                        ...
                        <div className="flex flex-wrap gap-2 mt-2">
                           <Link 
                              ...
                              className="px-3 py-1.5 rounded-lg bg-background border border-border text-foreground text-xs font-medium hover:border-primary-300 hover:text-primary-600 transition-colors shadow-sm"
                            >
                             + Quiz
                           </Link>
                           <Link 
                           ...
                           <Link to={`/courses/${c.id}`} className="px-3 py-1.5 rounded-lg bg-background border border-border text-foreground text-xs font-medium hover:border-primary-300 hover:text-primary-600 transition-colors shadow-sm">
                             Manage
                           </Link>
                        </div>
                      </div>
            ...
            <div className="rounded-2xl bg-card border border-border p-4">
              ...
              <div className="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3">
                {recentQuizzes.map(q => (
                  <div key={q.id} className="rounded-xl border border-border p-3 bg-gradient-to-b from-secondary/50 to-background">
                    <div className="font-medium text-foreground">{q.title}</div>
                    <div className="text-xs text-muted-foreground">{q.questionCount} questions • {q.completions} completions</div>
                    <div className="mt-2">
                      <div className="text-xs text-muted-foreground">Completion Rate</div>
                      <div className="mt-1 h-2 bg-secondary rounded-full overflow-hidden">
                        <div className="h-full bg-primary-600" style={{ width: `${q.passRate}%` }} />
                      </div>
                      <div className="text-right text-xs text-muted-foreground mt-1">{q.passRate}%</div>
                    </div>
                  </div>
                ))}
              </div>
              <div className="mt-4">
                <Link to="/instructor/quizzes/new" className="w-full inline-flex justify-center items-center rounded-xl bg-primary-600 text-white hover:bg-primary-700 transition-colors py-3">Create New Quiz</Link>
              </div>
            </div>
          </div>
          <aside className="space-y-4">
            <div className="rounded-2xl bg-card border border-border text-card-foreground p-4">
              <h2 className="font-semibold">Top Students</h2>
              <div className="mt-3 space-y-3">
                {topStudents.length === 0 && <div className="text-muted-foreground">No data yet.</div>}
                {topStudents.map((s, i) => (
