                            ...
                            <span className={`text-sm font-medium block ${activeLesson && activeLesson.id === lesson.id ? 'text-primary-700' : (isRegistered ? 'text-primary-900' : 'text-slate-700')}`}>{lesson.title}</span>
                             <div className="flex gap-2">
                               ...
                               {lesson.resource_path && (
                                   <a href={`${import.meta.env.VITE_API_BASE_URL}/lesson-download/${lesson.id}`} download target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()} className="text-xs text-blue-500 hover:text-blue-700 flex items-center gap-1">
