        ...
        <div className="w-full max-w-md">
           <div className="mb-6">
             ...
             <div className="bg-white dark:bg-card rounded-3xl shadow-xl p-8 sm:p-10 border border-slate-100 dark:border-border relative">
               <h2 className="text-3xl font-bold text-slate-800 dark:text-foreground">Welcome back</h2>
               <p className="mt-2 text-slate-500 dark:text-muted-foreground mb-8">Login to continue to your courses</p>
               ...
               <form onSubmit={onSubmit} className="space-y-5">
                 ...
                 <div className="flex items-center justify-between text-sm">
                    <label className="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" className="rounded border-slate-300 dark:border-input text-primary-600 focus:ring-primary-500 bg-white dark:bg-input" />
                        <span className="text-slate-600 dark:text-muted-foreground">Remember me</span>
                    </label>
                    <Link to="/forgot-password" className="text-primary-600 dark:text-primary-400 font-semibold hover:text-primary-700">Forgot password?</Link>
                 </div>

                 <button
                    type="submit"
                    disabled={loading}
                    className="w-full bg-slate-900 dark:bg-primary text-white dark:text-primary-foreground font-bold py-3.5 rounded-xl hover:bg-slate-800 dark:hover:bg-primary/90 transition-colors shadow-lg shadow-slate-200 dark:shadow-none mt-2 disabled:opacity-70 disabled:cursor-not-allowed"
                 >
                    {loading ? 'Logging in...' : 'Login'}
                 </button>

                 <div className="text-center mt-6">
                    <span className="text-slate-500 dark:text-muted-foreground text-sm">Don't have an account? </span>
                    <Link to="/signup" className="text-primary-600 dark:text-primary-400 font-bold text-sm hover:underline">Sign up for free</Link>
                 </div>
