...
function Bubble({ message, align, read }){
  ...
  const handleOtherReasonSubmit = async () => {
    ...
    <div className={`flex items-end gap-2 ${isRight?'justify-end':'justify-start'}`}>
      ...
      <div className={`max-w-[70%] rounded-2xl px-3 py-2 text-sm ${isRight?'bg-primary-600 text-white':'bg-white border border-slate-200'} ${read===false ? 'ring-2 ring-primary-400' : ''}`}>
      {message.text}
      {read===false && !isRight && (
        <span className="ml-2 text-xs text-primary-600 font-semibold">• Unread</span>
      )}
      </div>
    </div>
